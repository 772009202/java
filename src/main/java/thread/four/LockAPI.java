 ...
 *
 * getHoldCount实现类的api 当前线程调用lock()的次数
 * getQueueLength 等待此锁的线程的个数
 * getWaitQueueLength 返回此condition条件等待的线程
 */
public class LockAPI {
	...
	static class Thread1 extends Thread {
		...
		public void run() {
			super.run();
			/*service.change();*/ // getHoldCount的测试 2 1
			try {
	...
	static class Service {
		...
		void countWait () throws InterruptedException {
			lock.lock();
			/*Thread.sleep(1000);*/   //确保其他线程在上一线程完成前获得锁 不然结果就是0
